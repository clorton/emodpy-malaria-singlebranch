...
def set_config(config):
    ...
    # config.parameters.Custom_Individual_Events = ["ReceivedTreatment"]
    config.parameters.Report_Event_Recorder_Events = ["ReceivedTreatment", "NewInfectionEvent", "NewClinicalCase", "NewSevereCase"]
...
def set_param_fn(config, manifest, set_config_fn, duration=365, whokill="Male"):
    ...
    # Vector Genetics
    if whokill == "Male":
        malconf.add_resistance(manifest, "everybody_wants_some", "gambiae", [["X", "X"]])
        malconf.add_resistance(manifest, "everybody_wants_some", "gambiae", combo=[["X", "Y"]],
                               killing=0.0)
    elif whokill == "Female":
        malconf.add_resistance(manifest, "everybody_wants_some", "gambiae", [["X", "Y"]])
        malconf.add_resistance(manifest, "everybody_wants_some", "gambiae", combo=[["X", "X"]],
                               killing=0.0)

    elif whokill == "All":
        malconf.add_resistance(manifest, "everybody_wants_some", "gambiae", [["X", "Y"]], killing=0.0)
        malconf.add_resistance(manifest, "everybody_wants_some", "gambiae", combo=[["X", "X"]],
                               killing=0.0)

    elif whokill == "Mixed":
         malconf.add_resistance(manifest, "everybody_wants_some", "gambiae", [["X", "Y"]], killing=1.0)
         malconf.add_resistance(manifest, "everybody_wants_some", "gambiae", [["X", "X"]], killing=0.6)
