...
def set_param_fn(config): 
    ...
    malconf.add_resistance(manifest,
                           ...
                           combo=[["X", "X"]],
                           killing=0.0) # Makes female silly skeeters unaffected
...
def build_camp( actual_start_day=90, current_insecticide="kill_male_silly",
    ...
    from emodpy_malaria.interventions.outdoorrestkill import OutdoorRestKill as rest

    # This isn't desirable. Need to think about right way to provide schema (once)
    camp.schema_path = manifest.schema_file

    # print( f"Telling emod-api to use {manifest.schema_file} as schema." )
    camp.add(rest(camp, start_day=actual_start_day, target_coverage=coverage,
                  killing_effect=killing_effectiveness, killing_predecay_duration=730,
...
def general_sim( erad_path, ep4_scripts ):
    ...
    experiment  = Experiment.from_builder(builder, task,
                                          name="Malaria OutdoorRestKill male silly_skeeter")
